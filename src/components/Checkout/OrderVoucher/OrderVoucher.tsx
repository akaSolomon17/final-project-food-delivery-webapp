...
import { useGetVouchersList, useGetVouchersListByCode } from '../../../apis/vouchers/getVouchersList.api'
import { IVouchers } from '../../../types/vouchers.type';
import VoucherEmpty from './VoucherEmpty';
import useVoucher from '../../../hooks/useVouchers';
import { useCartActions, useIsVoucherApplied, useTotalPrice } from '../../../zustand/cartStore';
...
const OrderVoucher = () => {
    ...
    const { data: searchResult, isLoading } = useGetVouchersListByCode(searchKeyword)
    ...
    const debouncedSearch = debounce((value: string) => {
        ...
        if (value === '') {
            ...
            setActiveSearch(voucherListSearchData &&
                voucherListSearchData.filter((item: IVouchers) => item.code.includes(value))
            );
        }
    }, 500);
    ...
    useEffect(() => {
        if (searchKeyword === '') {
            ...
            setActiveSearch(
                ...
                voucherListSearchData.filter((item: IVouchers) => item.code.includes(searchKeyword))
            );
        }
    }, [searchKeyword]);
    ...
    return (
        <>
            <div className='min-w-[624px] w-[624px] h-[331px] bg-white mt-[24px] mb-[70px] mx-auto rounded-md'>
                <h2 className='font-semibold text-xl p-4'>Vouchers</h2>
                ...
                <span className='text-sm'>
                    {
                        appliedVouchers?.[0]?.description ?
                            <div className='flex justify-between items-center w-1/3 border-1 border-slate-300 bg-slate-200 p-2 rounded-sm ms-4'>
                                ...
                                <IoCloseCircleSharp size={18} className='cursor-pointer' onClick={handleCancelVoucher} />
                            </div> :
