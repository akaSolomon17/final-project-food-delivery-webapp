  ...
  actions: {
    ...
    setIsAppliedVoucher: (isVoucherApplied) => {
      ...
      localStorage.setItem(
        "isVoucherApplied",
        JSON.stringify(isVoucherApplied),
      );
    },
    loadFromLocalStorage: () => {
      const cart = JSON.parse(localStorage.getItem("cart") || "[]");
      const discountInfo = JSON.parse(
        localStorage.getItem("discountInfo") ||
          '{"priceReduce": 0, "totalPriceDiscount": 0}',
      );
      const isVoucherApplied = JSON.parse(
        localStorage.getItem("isVoucherApplied") || "false",
      );
...
export const useIsVoucherApplied = () =>
  cartStore((state) => state.isVoucherApplied);
